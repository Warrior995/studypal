...
import { getTopicsByUnit } from "@/app/api/topics/topicFunctions";
...
export default function UnitDetail({id} : {id: number}){
    ...
    useEffect(() => {
        (async () => {
            const topicsResponse = await getTopicsByUnit(id);
    ...
    return (
        <div className="flex flex-col gap-5 p-5 vh-full">
            <div className="flex items-center gap-5">
                <h1 className="text-[2rem] font-bold text-black pl-[2rem]">{unitName}</h1>
                <div className="flex-1"/>
                <button className="bg-gray-600 text-white px-4 py-2 rounded-lg mr-3 cursor-pointer min-w-[9rem]" onClick={() => router.push(`/units/${id}/studyMode`)}> Lets study</button>
                <button className="bg-gray-600 text-white px-4 py-2 rounded-lg mr-3 cursor-pointer min-w-[9rem]" onClick={() => router.push(`/units/${id}/practiceMode`)}> Lets practice</button>
                <button className="bg-blue-600 text-white px-4 py-2 rounded-lg mr-3 min-w-[9rem]" onClick={() => setCreateModalOpen(true)}> + Create Topic</button>
            </div>
            ...
            <NewTopicModal modalOpen={createModalOpen} setModalOpen={setCreateModalOpen} topics={topics} setTopics={setTopics} unitId={id}/>
